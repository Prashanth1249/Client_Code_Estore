...
function Home({ login, setLogin }) {
  ...
  const getSoldImages = async () => {
    try {
      ...
      const response = await axios.get("https://estorerguktbasarbackend.onrender.com/homebooks");
              ...
              <div className="book-list">
                {soldImages.map((image, index) => (
                  <div className="book-box" key={index}>
                    <img
                      src={`https://estorerguktbasarbackend.onrender.com/uploads/${image.imageName}`}
                      alt={image.title}
